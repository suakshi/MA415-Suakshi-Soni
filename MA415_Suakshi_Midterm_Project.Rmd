...
```{r income-summary-florida, echo=TRUE, message=FALSE}
# Histogram for Florida income
ggplot(income_cen_f, aes(x = Value)) +
  geom_histogram(binwidth = 50000, fill = "steelblue", color = "black", alpha = 0.7) +
  labs(title = "Income Distribution - Florida", x = "Income ($)", y = "Frequency") +
  theme_minimal()
...
```{r chemical-boxplot-california, echo=TRUE, message=FALSE}
# Chemical usage boxplot
ggplot(strawb_sur_c_3_chem, aes(x = as.factor(Year), y = Value, fill = Chemical)) +
  geom_boxplot() +
  labs(title = "Chemical Usage Over Years - California", x = "Year", y = "Value (lb/acre)") +
  theme_minimal()
``` Rising Chemical Usage in Conventional Farming
